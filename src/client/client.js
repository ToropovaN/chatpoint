...
const getRenderTime = () => {
  return (
    firstServerTimeStamp + (Date.now() - firstClientTimeStamp) - RENDER_DELAY
  );
};
...
export const getCurrentState = () => {
  ...
  if (base < 0 || base === renderUpdates.length - 1)
    currentState = renderUpdates[renderUpdates.length - 1];
  else {
    currentState = interpolateCurrentState(
      renderUpdates[base],
      renderUpdates[base + 1]
    );
...
const interpolateCurrentState = (baseState, nextState) => {
  ...
  const renderTime = getRenderTime();
  const ratio =
    (renderTime - baseState.timeStamp) /
    (nextState.timeStamp - baseState.timeStamp);
        ...
        if (basePositions[i][key] !== nextPositions[i][key]) {
          if (key === "rotW" || key === "rotY") {
            ...
            if (absD >= Math.PI) {
              ...
              interpolated.positions[i][key] =
                basePositions[i][key] +
                (nextPositions[i][key] - basePositions[i][key]) * ratio;
  ...
  socket.on("deletePlayer", (playerId) => {
    ...
    if (playerId === mySocketId) {
      ...
      if (index >= 0) {
        ...
        createMessage(
          newPlayer.nameColor,
          " -- " + newPlayer.name + " leave this chat! -- ",
          ""
        );
